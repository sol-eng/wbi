...
func newConfig(configOpts configOpts, item string) error {
	if item == "ssl" {
		...
		if configOpts.authType == "saml" {
			err := workbench.WriteSAMLAuthConfigWorkbench(configOpts.idpURL)
			if err != nil {
				return fmt.Errorf("failed to write SAML auth config for Workbench: %w", err)
			}
		} else if configOpts.authType == "oidc" {
			err := workbench.WriteOIDCAuthConfigWorkbench(configOpts.idpURL, configOpts.usernameClaim, configOpts.clientID, configOpts.clientSecret)
			if err != nil {
		...
		err := workbench.WriteRepoConfigWorkbench(configOpts.url, configOpts.source)
		if err != nil {
			return fmt.Errorf("failed to write repo config for Workbench: %w", err)
		}
	} else if item == "connect-url" {
		err := workbench.WriteConnectURLConfigWorkbench(configOpts.url)
		if err != nil {
